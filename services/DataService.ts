...
const CACHE_TTL_MS = 30 * 24 * 60 * 60 * 1000; // 30 days cache - ultra fast instant reload
...
const LOCAL_CACHE_TTL_MS = 30 * 24 * 60 * 60 * 1000; // 30 days for localStorage - instant reload on revisit
...
class DataServiceImpl {
  ...
  async getAllReviews(tenantId: string) {
    ...
    const response = await this.requestTenantApi(`/api/reviews/${tenantId}/all`, {
      headers: {
        'Authorization': `Bearer ${token}`
      }
    });
  ...
  async updateReviewStatus(reviewId: string, status: 'approved' | 'rejected', tenantId: string) {
    const token = this.getToken();
    if (!token) {
      throw new Error('Authentication required');
    }
