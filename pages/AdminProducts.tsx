...
import { Search, Plus, Edit, Trash2, X, Upload, Save, Image as ImageIcon, CheckCircle, AlertCircle, Grid, List, CheckSquare, Layers, Tag as TagIcon, Percent, Filter, RefreshCw, Palette, Ruler, ChevronDown, Maximize2, Square, Grip, Table, Loader2, FileEdit, Copy, MoreVertical, Eye, ExternalLink, FolderOpen, Download } from 'lucide-react';
...
const AdminProducts: React.FC<AdminProductsProps> = ({ 
  ...
  const [useNewModal, setUseNewModal] = useState(true); // Toggle for new Figma design modal
  ...
  const isListLikeView = viewMode === 'list';  // details mode now uses table view
    ...
    <div className="space-y-3 sm:space-y-4 animate-fade-in relative">
          ...
          <div className="hidden lg:block overflow-x-auto overflow-y-visible">
            <table className="w-full">
              <thead className="h-12 bg-gradient-to-b from-sky-400/25 to-blue-500/25">
                <tr>
                  <th className="px-4 py-3 text-left text-base font-medium text-black w-12">
                    <input
                      type="checkbox"
                      checked={selectedIds.length === paginatedProducts.length && paginatedProducts.length > 0}
                      onChange={() => {
                  ...
                  <th className="px-4 py-3 text-left text-base font-medium text-black w-16">SL</th>
                  <th className="px-4 py-3 text-left text-base font-medium text-black w-20">Image</th>
                  <th className="px-4 py-3 text-left text-base font-medium text-black min-w-[200px]">Name</th>
                  <th className="px-4 py-3 text-left text-base font-medium text-black">Category</th>
                  <th className="px-4 py-3 text-left text-base font-medium text-black">Sub Category</th>
                  <th className="px-4 py-3 text-left text-base font-medium text-black w-20">Priority</th>
                  <th className="px-4 py-3 text-left text-base font-medium text-black">SKU</th>
                  <th className="px-4 py-3 text-left text-base font-medium text-black">Tags</th>
                  <th className="px-4 py-3 text-left text-base font-medium text-black w-24">Status</th>
                  <th className="px-4 py-3 text-center text-base font-medium text-black w-20">Action</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-zinc-400">
                {paginatedProducts.map((product, index) => {
                  ...
                  return (
                    <tr 
                      key={productKey}
                      className={`hover:bg-gray-50 transition ${isSelected ? 'bg-[#E6F7F5]' : ''} ${isDraftProduct ? 'border-l-4 border-l-yellow-500' : ''}`}
                    >
                      ...
                      <td className="px-4 py-3">
                        <span className="text-xs font-medium text-[#2DD4BF] bg-[#E6F7F5] px-2 py-1 rounded-full">
                          {product.category || '—'}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                 ...
                 <div className="flex flex-col gap-4 p-4 md:flex-row md:items-center md:justify-between">
                   <div className="flex items-center gap-3 flex-1 min-w-0">
                     <input 
                       ...
                       checked={isSelected}
                       onChange={() => toggleSelection(product.id)}
                       className="w-5 h-5 text-[#2DD4BF] rounded border-gray-300 focus:ring-[#2DD4BF] cursor-pointer flex-shrink-0"
                     />
                     ...
                     <div className="flex-1 min-w-0 space-y-1 cursor-pointer" onClick={() => toggleSelection(product.id)}>
                        <div className="flex flex-wrap gap-1">
                         <span className="text-[11px] font-medium text-[#2DD4BF] bg-[#E6F7F5] px-2 py-0.5 rounded-full">
             ...
             >
              <div className="absolute top-3 left-3 z-10 flex items-center gap-2">
                <input 
                  ...
                  checked={isSelected}
                  onChange={() => toggleSelection(product.id)}
                   className="w-5 h-5 text-[#2DD4BF] rounded border-gray-300 focus:ring-[#2DD4BF] shadow-sm cursor-pointer"
                />
              </div>
              ...
              <div className={`relative ${imageHeightClass} bg-gray-100`}>
                 ...
                 <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition flex items-center justify-center gap-3 backdrop-blur-[1px]">
                    <button 
                     ...
                     className="bg-white text-gray-800 p-2 rounded-full hover:bg-[#E6F7F5] hover:text-[#2DD4BF] transition shadow-lg"
                      title="Edit"
                    >
                      <Edit size={18} />
                    </button>
              ...
              <div className={`${viewMode === 'smallIcons' ? 'p-3' : viewMode === 'extraLargeIcons' ? 'p-5' : 'p-4'} cursor-pointer`} onClick={() => toggleSelection(product.id)}>
                 <div className="mb-2 flex flex-wrap gap-1">
                   <span className={`${viewMode === 'smallIcons' ? 'text-[10px]' : 'text-xs'} font-medium text-[#2DD4BF] bg-[#E6F7F5] px-2 py-0.5 rounded-full`}>
                      {product.category || 'Uncategorized'}
                    </span>
                    {product.brand && (
      ...
      {/* Add/Edit Modal - New Figma Design */}
      {isModalOpen && useNewModal && (
        <ProductFormModal
          isOpen={isModalOpen}
          onClose={() => {
            setIsModalOpen(false);
      ...
      {/* Add/Edit Modal - Legacy (for fallback) */}
      {isModalOpen && !useNewModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-2 sm:p-4 animate-in fade-in duration-200">
           <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[95vh] sm:max-h-[90vh] flex flex-col">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-2">
                              ...
                              <input 
                                type="text" 
                                required
                            className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#2DD4BF] focus:outline-none"
                                value={formData.name}
                            ...
                            <div className="space-y-2">
                              ...
                              <input 
                                type="text" 
                            className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#2DD4BF] focus:outline-none text-sm"
                                value={formData.slug || ''}
                                onChange={e => {
                                  setIsSlugTouched(true);
                            ...
                            <div className="space-y-2">
                              ...
                              <select 
                            className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#2DD4BF] focus:outline-none bg-white"
                      ...
                      {expandedSections.images && (
                       ...
                       {!formData.galleryImages || formData.galleryImages.length === 0 ? (
                          <div className="space-y-3">
                            ...
                            className="border-2 border-dashed border-gray-300 rounded-xl h-32 flex flex-col items-center justify-center cursor-pointer hover:border-purple-500 hover:bg-[#E6F7F5] transition group"
                            >
                            <div className="bg-purple-100 p-2 rounded-full text-[#2DD4BF] mb-2 group-hover:scale-110 transition">
                                 <Upload size={20} />
                               </div>
                             ...
                             {formData.galleryImages.length < 10 && (
                                <div className="flex flex-col sm:flex-row gap-2">
                                  <button 
                                    type="button"
                                    onClick={() => fileInputRef.current?.click()}
                                  className="flex-1 py-2 sm:py-2.5 border-2 border-dashed border-gray-300 rounded-lg text-xs sm:text-sm font-medium text-gray-600 hover:border-purple-500 hover:text-[#2DD4BF] transition flex items-center justify-center gap-2"
                                  >
                        ...
                        <div className="p-3 sm:p-4 space-y-4 bg-white">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-2">
                              ...
                              className="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#2DD4BF] focus:outline-none text-sm"
                                  value={colorInput}
                    ...
                    <div className="border border-gray-200 rounded-lg overflow-hidden">
                      ...
                      {expandedSections.tags && (
                        <div className="p-3 sm:p-4 space-y-4 bg-white">
                          ...
                          <div className="flex flex-wrap gap-2 sm:gap-3 pb-4 border-b border-gray-200">
                            ...
                            <button
                              ...
                              className={`flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 rounded-lg border-2 font-medium transition-all text-sm sm:text-base ${
                                ...
                                ? 'bg-[#E6F7F5] border-purple-400 text-purple-700'
                            ...
                            >
                              ...
                              <span>Popular</span>
                              {formData.tags?.some(t => t.toLowerCase() === 'popular') && (
                          <span className="bg-[#E6F7F5]0 text-white text-xs px-1.5 py-0.5 rounded">ON</span>
                              )}
                            </button>
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex gap-2">
                              <input 
                                type="text" 
                                placeholder="Add tag..."
                            className="flex-1 px-3 sm:px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#2DD4BF] focus:outline-none text-sm sm:text-base"
                                value={tagInput}
                            ...
                            <div className="flex gap-2 flex-wrap mt-2">
                              {tags.map(t => (
                                <button 
                                  ...
                                  className="text-xs bg-gray-50 border border-gray-200 px-2 py-1 rounded-full text-gray-600 hover:bg-[#E6F7F5] hover:text-[#2DD4BF] hover:border-purple-200 transition"
                                >
                                  + {t.name}
                                </button>
                              ))}
                          ...
                          <Search size={16} className="text-[#2DD4BF]" />
                            ...
                            className="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#2DD4BF] focus:outline-none text-sm"
                              ...
                              />
                          <button type="button" onClick={addSearchTag} className="bg-purple-100 px-4 rounded-lg hover:bg-purple-200 text-[#2DD4BF] font-bold">+</button>
                            </div>
              ...
              {/* Saving Progress Bar */}
              {isSaving && (
              <div className="px-4 sm:px-6 py-3 bg-[#E6F7F5] border-t border-purple-100">
              ...
              <div className="p-4 sm:p-6 border-t border-gray-100 bg-gray-50 rounded-b-2xl flex flex-col-reverse sm:flex-row justify-end gap-2 sm:gap-3">
                 ...
                 <button 
                   ...
                   form="productForm"
                   disabled={isSaving}
                className="w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2.5 bg-gradient-to-r from-[#38BDF8] to-[#1E90FF] text-white rounded-lg transition text-sm font-medium shadow-lg hover:from-[#2BAEE8] hover:to-[#1A7FE8] disabled:opacity-70 disabled:cursor-not-allowed"
                 >
                   {isSaving ? (
                     <>
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
           <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm overflow-hidden">
              ...
              <div className="p-6">
                ...
                <button onClick={executeBulkAction} className="flex-1 py-2 bg-gradient-to-r from-[#38BDF8] to-[#1E90FF] text-white rounded-lg hover:from-[#2BAEE8] hover:to-[#1A7FE8] font-bold shadow-md">Apply</button>
                 </div>
              </div>
           </div>
        </div>
