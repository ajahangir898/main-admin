...
const AdminControl: React.FC<AdminControlProps> = ({
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-white flex items-center gap-2">
            <Shield className="text-emerald-400" /> Admin Control
          </h2>
      ...
      {tab === 'roles' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {roles.filter(r => r._id || r.id).map(role => {
            ...
            return (
              <div key={id} className="bg-white/5 rounded-xl border border-white/10 p-5 hover:border-emerald-500/30 transition group">
                <div className="flex justify-between items-start mb-3">
                  ...
                  {!role.isSystem && canModifyRoles && (
                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition">
                      <button onClick={() => openRoleModal(role)} className="p-1.5 hover:bg-white/10 rounded text-slate-400 hover:text-emerald-400"><Edit size={14} /></button>
                      {canDeleteRoles && (
                        <button onClick={() => deleteRole(role)} className="p-1.5 hover:bg-white/10 rounded text-slate-400 hover:text-red-400"><Trash2 size={14} /></button>
                      )}
                    </div>
                  )}
                </div>
                <p className="text-slate-500 text-sm mb-4 line-clamp-2">{role.description || 'No description'}</p>
                <div className="flex gap-4 text-xs text-slate-500 pt-3 border-t border-white/10">
                  <span><Users size={12} className="inline mr-1" />{userCount} users</span>
                  <span><Shield size={12} className="inline mr-1" />{permCount} perms</span>
                </div>
              </div>
            );
          })}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-slate-900 rounded-2xl w-full max-w-md border border-white/10">
            <div className="p-5 border-b border-white/10 flex justify-between items-center">
              ...
              <button onClick={() => setUserModal(false)} className="text-slate-400 hover:text-white p-1"><X size={18} /></button>
            </div>
            ...
            <form onSubmit={saveUser} className="p-5 space-y-4">
              <div>
                ...
                <input type="text" required value={userForm.name} onChange={e => setUserForm({...userForm, name: e.target.value})}
                  className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-lg text-white text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/50" placeholder="John Doe" />
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="email" required disabled={!!editUser} value={userForm.email} onChange={e => setUserForm({...userForm, email: e.target.value})}
                    className="w-full pl-9 pr-3 py-2.5 bg-white/5 border border-white/10 rounded-lg text-white text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/50 disabled:opacity-50" placeholder="john@example.com" />
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type={showPwd ? 'text' : 'password'} required minLength={6} value={userForm.password} onChange={e => setUserForm({...userForm, password: e.target.value})}
                      className="w-full pl-9 pr-9 py-2.5 bg-white/5 border border-white/10 rounded-lg text-white text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/50" placeholder="Min 6 chars" />
                    <button type="button" onClick={() => setShowPwd(!showPwd)} className="absolute right-3 top-3 text-slate-500 hover:text-white">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="tel" value={userForm.phone} onChange={e => setUserForm({...userForm, phone: e.target.value})}
                    className="w-full pl-9 pr-3 py-2.5 bg-white/5 border border-white/10 rounded-lg text-white text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/50" placeholder="+880 1XXX" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select value={userForm.roleId} onChange={e => setUserForm({...userForm, roleId: e.target.value})}
                    className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-lg text-white text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/50">
              ...
              <div className="flex gap-3 pt-3">
                <button type="button" onClick={() => setUserModal(false)} className="flex-1 py-2.5 border border-white/20 rounded-lg text-slate-400 hover:text-white hover:bg-white/5">Cancel</button>
                <button type="submit" disabled={loading} className="flex-1 py-2.5 bg-emerald-500 hover:bg-emerald-600 rounded-lg text-white font-medium flex items-center justify-center gap-2 disabled:opacity-50">
      ...
      {roleModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-slate-900 rounded-2xl w-full max-w-3xl max-h-[90vh] flex flex-col border border-white/10">
            <div className="p-5 border-b border-white/10 flex justify-between items-center">
              <h3 className="text-lg font-semibold text-white flex items-center gap-2">
                <Key size={20} className="text-emerald-400" />
                {editRole ? 'Edit Role' : 'Create Role'}
              </h3>
              <button onClick={() => setRoleModal(false)} className="text-slate-400 hover:text-white p-1"><X size={18} /></button>
            </div>
            
            <form id="roleForm" onSubmit={saveRole} className="flex-1 overflow-y-auto p-5 space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-sm text-slate-400 block mb-1">Name *</label>
                  <input type="text" required value={roleForm.name} onChange={e => setRoleForm({...roleForm, name: e.target.value})}
                    className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-lg text-white text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/50" placeholder="Order Manager" />
                </div>
                <div>
                  ...
                  <input type="text" value={roleForm.description} onChange={e => setRoleForm({...roleForm, description: e.target.value})}
                    className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-lg text-white text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/50" placeholder="Brief description..." />
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex gap-2 text-xs text-slate-500">
                    {ACTIONS.map(a => <span key={a} className="px-1.5 py-0.5 bg-white/5 rounded">{a[0].toUpperCase()}</span>)}
                  </div>
                </div>
                ...
                <div className="bg-white/5 rounded-xl border border-white/10 divide-y divide-white/10">
                  {Object.entries(groupedResources).map(([group, resources]: [string, { id: ResourceType; label: string; group: string }[]]) => (
                    <div key={group}>
                      <div className="px-4 py-2 bg-white/5 text-xs font-semibold text-emerald-400 uppercase">{group}</div>
                      <div className="divide-y divide-white/5">
                        {resources.map(r => {
                          const perm = roleForm.permissions.find(p => p.resource === r.id);
                          const count = perm?.actions.length || 0;
                          return (
                            <div key={r.id} className="px-4 py-2.5 flex items-center justify-between hover:bg-white/5">
                              <div className="flex items-center gap-2">
                                <button type="button" onClick={() => toggleAllPerms(r.id)}
                                  className={`w-5 h-5 rounded flex items-center justify-center text-xs border transition ${count === 4 ? 'bg-emerald-500 border-emerald-500 text-white' : count > 0 ? 'bg-emerald-500/30 border-emerald-500/50 text-emerald-400' : 'bg-white/5 border-white/20 text-slate-500'}`}>
            ...
            <div className="p-5 border-t border-white/10 flex gap-3">
              <button type="button" onClick={() => setRoleModal(false)} className="flex-1 py-2.5 border border-white/20 rounded-lg text-slate-400 hover:text-white hover:bg-white/5">Cancel</button>
              <button type="submit" form="roleForm" disabled={loading} className="flex-1 py-2.5 bg-emerald-500 hover:bg-emerald-600 rounded-lg text-white font-medium flex items-center justify-center gap-2 disabled:opacity-50">
                {loading ? <Loader2 size={16} className="animate-spin" /> : <Save size={16} />}
